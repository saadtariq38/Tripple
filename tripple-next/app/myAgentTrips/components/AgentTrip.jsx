...
import cancelTrip from "@/lib/cancelTrip";


export default function AgentTrip(props) {
    ...
    async function deleteTripClick(tripId) {
        try {
            ...
            if (error.message === "Unauthorized-only traveller can unregister from trips") {
              ...
              toast.error('Only agents can cancel trips!', {
                position: "bottom-center",
                autoClose: 5000,
                hideProgressBar: false,
                closeOnClick: true,
              ...
              await cancelTrip(tripId, newAccessToken)
                .then((data) => {
                  //setProgress("Unregistered successfully")
                  toast.success('trip cancelled successfully!', {
                    position: "bottom-center",
    ...
    <div className="flex flex-row">
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 lg:max-w-5xl lg:gap-x-20 lg:grid-cols-2 mt-16 mr-24">
        <div className="relative p-3 col-start-1 row-start-1 flex flex-col-reverse rounded-lg bg-gradient-to-t from-black/75 via-black/0 sm:bg-none sm:row-start-2 sm:p-0 lg:row-start-1">
          <h1 className="mt-1 text-lg font-semibold text-white sm:text-slate-900 md:text-2xl dark:sm:text-white">{props.name}</h1>
          <p className="text-lg leading-4 font-medium text-white sm:text-slate-500 dark:sm:text-slate-400">Pkr {props.cost}/head</p>
        </div>
        ...
        <dl className="mt-4 text-xs font-medium flex items-center row-start-2 sm:mt-1 sm:row-start-3 md:mt-2.5 lg:row-start-2">
          ...
          <dd className="text-white flex items-center dark:text-indigo-400">
        ...
        <div className="mt-4 col-start-1 row-start-3 self-center sm:mt-0 sm:col-start-2 sm:row-start-2 sm:row-span-2 lg:mt-6 lg:col-start-1 lg:row-start-3 lg:row-end-4">
          <Link href='/agentTripDetails'>
            <button onClick={() => handleTripClick(props)} type="button" className="bg-blue-600 text-white text-sm leading-6 font-medium py-2 px-3 rounded-lg">
              More Info
            </button>
          </Link>
        </div>
      ...
      {props.status === 'cancelled' ? (
          <h1 class="flex items-center justify-center underline underline-offset-3 mb-4 text-xl font-extrabold leading-none tracking-tight text-red-700 md:text-3xl lg:text-4xl dark:text-white">Deleted</h1>
      ) : (
